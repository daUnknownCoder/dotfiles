...
export const PannelButton = props =>
    Widget.Button({
        ...props,
...
export const Temperature = props =>
    Widget.Label({
        ...
        setup: self =>
            self.hook(Weather, () => {
                if (
                    Weather.temperatureWeather &&
                    self.label !== Weather.temperatureWeather
                ) {
...
export const WeatherInfo = weatherData =>
    Widget.Box({
        ...
        children: [
            Widget.Label({
                label: weatherData.date.substring(0, 3).toUpperCase(),
            }),
            Widget.Label({ label: weatherData.hour + 'h' }),
            Widget.Label({
                label: weatherData.icon,
                class_name: 'weather-icon',
            }),
            Widget.Label({
                label: weatherData.temperature,
                class_name: 'weather-temperature',
            }),
            Widget.Box({ vexpand: true }),
            weatherData.rain !== '0 mm'
                ? Widget.Label({ label: weatherData.rain })
                : null,
            // Widget.Label({ label: " " + weatherData.wind, }),
            // Widget.Label({ label: weatherData.humidity, }),
            Widget.Label({
                label: '↑ ' + weatherData.maxTemp,
                class_name: 'weather-max',
            }),
            Widget.Label({
                label: '↓ ' + weatherData.minTemp,
                class_name: 'weather-min',
            }),
        ],
        setup: self =>
            self.hook(Weather, () => {
...
export const WeatherBoxChild = w =>
    Widget.Box({
        ...
        children: [
            ...
            w.rain !== '0 mm'
                ...
                : null,
            Widget.Label({
                label: '  ' + Math.round(w.wind.replace(/kph$/, '')) + ' kph',
                class_name: 'weather-wind',
            }),
        ],
    });

export const WeatherBoxChildWrapper = (
    w,
            ...
            self.hook(Weather, () => {
                ...
                if (totalWeatherForecastDataArray.length) {
                    ...
                    totalWeatherForecastDataArray.forEach(totalEl => {
                        self.add(WeatherBoxChild(totalEl));
                    });
                }

                if (
                    false === useTotalInstead &&
                    w &&
                    temperatureDataPerDay &&
                    temperatureDataPerDay[w.date.substring(0, 3).toUpperCase()]
                        .data.length
                ) {
                    temperatureDataPerDay[
                        w.date.substring(0, 3).toUpperCase()
                    ].data.forEach(el => {
                        self.add(WeatherBoxChild(el));
                    });
                }
            }),
    });

export const WeatherMainWidget = (
    widgetIcon,
    widgetDate,
    rain,
    temperatureDataPerDay,
                ...
                children: [
                    ...
                    Widget.Label({
                        label:
                            '↑ ' +
                            temperatureDataPerDay[
                                widgetDate.substring(0, 3).toUpperCase()
                            ].maxTemp +
                            '  ',
                        class_name: 'weather-max',
                    }),
                    ...
                    Widget.Label({
                        label: widgetDate.substring(0, 3).toUpperCase(),
                        class_name: 'weather-hour',
                    }),
                ],
            }),
            WeatherBoxChildWrapper(
                w,
                temperatureDataPerDay,
                totalWeatherForecastDataArray
            ),
...
export const Tooltip = (total = null) =>
    Widget.Box({
        setup: self =>
            self.hook(Weather, () => {
                ...
                if (tooltip) {
                    ...
                    tooltip.forEach(w => {
                        ...
                        if (
                            total &&
                            totalWeatherForecastsCounter &&
                            totalWeatherForecastsCounter > 0
                        ) {
                        ...
                        if (!temperatureDataPerDay[date]) {
                            ...
                            temperatureDataPerDay[date].minTemp = Math.min(
                                temperatureDataPerDay[date].minTemp,
                                temperature
                            );
                            temperatureDataPerDay[date].maxTemp = Math.max(
                                temperatureDataPerDay[date].maxTemp,
                                temperature
                            );
                            temperatureDataPerDay[date].rain = Math.max(
                                temperatureDataPerDay[date].rain,
                                rain
                            );
                            temperatureDataPerDay[date].icons =
                                weatherStatusIconArray;
                            temperatureDataPerDay[date].widgetsNumber =
                                forecastWidgetsNumber;
                            temperatureDataPerDay[date].data =
                                weatherForecastDataArray;
                        }
