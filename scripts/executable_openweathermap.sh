#!/usr/bin/bash
output=""
apiKey=$(pass API_keys/openweather)
cityname=$(curl -s ipinfo.io/city)
defaultLocation=$(curl -s "http://api.openweathermap.org/data/2.5/find?q=$cityname&appid=$apiKey" | jq -r '.list[0].id')
Ags=0
Debug=0
WindSpeedUnit="kph"
DegreeCharacter="°C"
