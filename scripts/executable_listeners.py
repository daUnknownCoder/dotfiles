...
def read_shell(
    ...
    if p.returncode != 0:
        ...
        if retry and _attempts < 5:
            time.sleep(1)
            print(
                f"Retrying {command if isinstance(command, str) else ' '.join(command)} ... {_attempts}",
                file=sys.stderr,
            )
            return read_shell(
                command,
...
class BaseListener:
    def __init__(
        self, follower_command: list | str, follower_pattern: str, shell: bool = False
    ) -> None:
    ...
    def listen(self) -> None:
        with subprocess.Popen(
            self.follower_command, stdout=subprocess.PIPE, shell=self.shell
        ) as p:  # TODO maybe use this to get info than just a hook? meh
...
class SignalListener(BaseListener):
    ...
    def read(self) -> dict | list:
        print(
            f"Invalid Operation, --listen must be provided for '{args.command}'",
            file=sys.stderr,
        )
        return {}
...
class CachedIntervalListener(BaseListener):
    def __init__(self, identifier: str, interval: int) -> None:
        ...
        self.cache_path = Path("~/.config/data/cache.json").expanduser()
    ...
    def write_cache(self) -> tuple[float, dict | list]:
        ...
        with open(self.cache_path, "w") as fw:
            ...
            cache = self.read()
            content[self.identifier] = {"timestamp": timestamp, "cache": cache}
...
class Weather(CachedIntervalListener):
    ...
    def read(self) -> dict:
        ...
        with requests.get(
            f"http://api.openweathermap.org/geo/1.0/direct?q={self.location}&limit=1&appid={self.api}"
        ) as r:
        ...
        with requests.get(
            f"https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&appid={self.api}"
        ) as r:
        ...
        image = Path("~/.config/images/weather.png").expanduser()
        with requests.get(
...
class Schedule(SignalListener):
    def __init__(self) -> None:
        ...
        self.registered.extend([self.fetch, self.add_event, self.remove_event])
        self.path = Path("~/.config/data/schedule.json").expanduser()
    ...
    def fetch(self, week):
        ...
        for time, event in self._schedule[week.lower()].items():
            buff.append({"time": time, "event": event})
    ...
    def add_event(self, _):
        data = read_shell(
            [
                ...
                "--add-list=Select Day",
                "--list-values=sunday|monday|tuesday|wednesday|thursday|friday|saturday",
            ],
            ignore_error=True,
        ).split("|")
    ...
    def remove_event(self, data):
        ...
        try:
            ...
            print(
                f"Could not delete schedule with the following data: {data}",
                file=sys.stderr,
            )
...
class Wifi(SignalListener):
    ...
    def scan(self, _) -> None:
        ...
        for device in dict.fromkeys(
            read_shell(["nmcli", "-t", "-f", "SSID", "dev", "wifi", "list"]).split("\n")
        ):
            buff.append({"ssid": device})
        self.dispatch(buff)
...
class Bluetooth(SignalListener):
    def __init__(self) -> None:
        ...
        self.registered.extend([self.scan])
    ...
    def scan(self, _) -> None:
        ...
        try:
            ...
            for addr, name in devices:
                buff.append({"addr": addr, "name": name})
        except OSError:
            pass
...
class Power(BaseListener):
    def __init__(self) -> None:
        super().__init__(
            ["journalctl", "-fu", "lenovo-profile.service"], "(.+lenovo-profile)"
        )

    def read(self) -> dict | list:
        status = read_shell(
            ["journalctl", "-u", "lenovo-profile.service", "-n", "1"]
        ).split()[-1]

        return {"status": status, "glyph": self.glyph(status)}
...
class Workspaces(BaseListener):
    def __init__(self) -> None:
        super().__init__(
            [
                "socat",
                "-u",
                f"UNIX-CONNECT:/tmp/hypr/{os.environ['HYPRLAND_INSTANCE_SIGNATURE']}/.socket2.sock",
                "-",
            ],
            "(workspace)",
        )
    ...
    def read(self):
        time.sleep(
            0.1
        )  # weird bug in hyprctl causes same pinned windows to show up twice in `hyprctl workspaces` yeeeeeeee
...
class Network(BaseListener):
    def __init__(self) -> None:
        super().__init__(
            "nmcli monitor & journalctl -fu bluetooth.service",
            "(.+\\sconnected|.+\\sdisconnected|Connectivity|.+bluetoothd)",
            shell=True,
        )
    ...
    def read(self) -> list | dict:
        ...
        buff["bluetooth"] = (
            read_shell(
                "bluetoothctl info | grep 'Name' | sed 's/\\sName:\\s//g'", shell=True
            )
            or None
        )
        buff["ssid"] = None
...
class Player(BaseListener):
    def __init__(self) -> None:
        super().__init__(
            ["dbus-monitor"], ".+(\\/org\\/mpris\\/MediaPlayer2.+PropertiesChanged)"
        )
    ...
    def read(self) -> dict | list:
        ...
        players = (
            [args.player]
            if args.player
            else read_shell(["playerctl", "--list-all"], ignore_error=True).split()
        )
        if not players:
            return buff
        ...
        for player in players:
            data = read_shell(
                ...
                retry=True,
                ignore_error=True,  # idk why but it fails randomly woo
            )
            ...
            buff["alive"].append(
                {
                    ...
                    "album": album,
                    "url": url,
                    "art_url": self.playerart(art_url)
                    or f'file://{Path("~/.config/eww/images/playerart-default.png").expanduser()}',
                    "length": length,
                }
            )
    ...
    def playerart(self, url) -> str | None:
        ...
        with requests.get(url) as r:
            ...
            path = Path("~/.config/images/playerart.png").expanduser()
    ...
    def brief(self, data: list, output) -> list:
        ...
        for device in data:
            try:
                volvar = (
                    int(
                        device.get("volume").get("front-left").get("value_percent")[:-1]
                    ),
                )
            except AttributeError:
                volvar = (
                    int(device.get("volume").get("mono").get("value_percent")[:-1]),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--listen", help="Listen for changes", action="store_true"
    )
    parser.add_argument(
        "-pp", "--pprint", help="Pretty print output", action="store_true"
    )

    audio = subparser.add_parser("audio")
    audio.add_argument(
        "-d", "--default", help="Show default sink and source", action="store_true"
    )
    ...
    network.add_argument(
        "-t", "--types", help="Select device type", nargs="+", default=[]
    )

    weather = subparser.add_parser("weather")
    weather.add_argument(
        "-l", "--location", help="The location to fetch data for", required=True
    )
